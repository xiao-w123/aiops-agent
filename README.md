...
使用 30 条分层测试用例进行评测（L1-L5，每层 6 条），并对比了"无证据校验"与"含证据校验"两种配置。
...
| 配置 | 关键词命中 | 根因定位 | L5 根因 | 平均耗时 |
| :--- | :--- | :--- | :--- | :--- |
| 无证据校验 | 97% | 67% | 17% | 3.6s |
| **含证据校验（本方案）** | **95%** | **83%** | **100%** | **5.3s** |

**关键发现**：引入证据校验后，L5（边界与拒答）层的根因定位准确率从 **17% 提升到 100%**，总体根因定位准确率从 **67% 提升到 83%**。代价是平均耗时从 3.6s 增加到 5.3s（约 47% 的性能开销）。这验证了证据校验在抑制过度诊断方面的有效性。
